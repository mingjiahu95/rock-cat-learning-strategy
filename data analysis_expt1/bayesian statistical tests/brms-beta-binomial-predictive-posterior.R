...
# violin plots
cond_labels <- c("GEE" = "Passive\nSampling", "CE" = "Active\nSelection") #customize label names
  ...
  for (version_value in levels(df$version)){
    ...
    p <- ggplot(df_obs_pred, aes(x = cond, fill = cond)) +
      ...
      geom_dotplot(aes(y = Pr_corr),
                   ...
                   fill = NA, color = "darkslategray",
                   binwidth = 1/48, dotsize = 1, stackratio = 1.2) +
      geom_segment(mapping = aes(x = as.numeric(cond) - 0.2, xend = as.numeric(cond),
                                 y = PrCorr_mean, yend = PrCorr_mean),
                   data = df_obs_summary) +
    ...
    ggsave(filename, plot = p, path = pathname, width = 9, height = 6)
  }
}
